...
if "Visual Studio Code" in jp_windows.active_window_name():
    ...
    dest = "e:\\xcopy_backups\\" + to_international_date(now())
    # source = "c:\\docs\\"
    # source = "c:\\docs\\pythoncode\\"
    # source = "c:\\users\\john\\"
else:
    source = sys.argv[1]
    dest = slash(sys.argv[2]) + to_international_date(now())

call(["xcopy", source, dest, "/r", "/i", "/c", "/h", "/k", "/y", "/m", "/s"]) # I just removed the "/e" switch as I don't want empty folders
